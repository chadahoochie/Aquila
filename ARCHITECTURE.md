...
4. **Async-First Execution**: The query subsystem is strictly non-blocking and asynchronous (`QueryAsync<T>()`, `QueryPagedAsync<T>()`, `StreamAsync<T>()`), preventing sync-over-async thread pool starvation in high-throughput web applications.
