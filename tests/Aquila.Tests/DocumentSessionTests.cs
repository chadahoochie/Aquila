...
public sealed class DocumentSessionTests
{
        ...
        SampleDocument doc2)
    {
        ...
        {
            new DocumentEnvelope<SampleDocument> { Id = doc1.Id, PartitionKey = nameof(SampleDocument), DocType = nameof(SampleDocument), Data = doc1 },
        ...
        SampleDocument document)
    {
        ...
        await docStorage.Received(1).ExecuteBatchAsync(
            Arg.Is<IEnumerable<StorageOperation>>(ops => System.Linq.Enumerable.Any(ops, op => op.Id == document.Id)),
        ...
        IDocumentStorageProvider docStorage)
    {
        ...
        // Act
        session.Store(doc);
        doc.Title = "Mutated Title"; // Mutate object AFTER Store() call
        await session.SaveChangesAsync(TestContext.Current.CancellationToken);

        // Assert - The stored envelope data must retain "Original Title"
        await docStorage.Received(1).ExecuteBatchAsync(
